...
static bool write(struct list_t* lst, FILE* file) {
    struct list_t* it = lst;
    ...
    while (it != NULL) {
        if (fwrite(&it->value, sizeof(int), 1, file) < 1) {
            return false;
        }
        it = it->next;
    }
...
static bool read(struct list_t** lst, FILE* file) {
    size_t length;
    size_t i;
    int value;
